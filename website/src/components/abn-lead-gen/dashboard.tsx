...
import {
  type DashboardAdmin, type DashboardView, type Job, type Lead, type LeadSource, type ReportKind,
  dateLabel, formatABN, humanize, jobActive, safeReportURL, sourceName, sourceRunAllowed,
} from "@/lib/abn-lead-gen/types";
import { errorMessage, useDashboard, type DashboardController } from "./use-dashboard";
import { LeadActions, SourceReviews } from "./live-workflow";
import { readinessGuidance, sourceJobGuidance } from "./readiness";
...
function RunButton({ engine }: { engine: DashboardController }) {
  ...
  const allowed = sourceRunAllowed(engine.data);
  return <button type="button" className={cx("button", "primary", "run-button")} disabled={!engine.connected || !allowed || active || engine.saving || engine.signingOut} onClick={() => void engine.beginRun()}><Icon name={active ? "clock" : "play"} /><span>{active ? "Run in progress" : engine.data ? engine.data.mode === "fixture" ? "Run demo" : allowed ? `Check ${sourceName(engine.data.settings.default_source)} updates` : "Check source setup" : "Connect engine to run"}</span></button>;
}
...
function QbccAttribution({ engine }: { engine: DashboardController }) {
  const data = engine.data;
  if (!data || data.mode === "fixture" || !(data.sources.some(source => source.source === "qbcc" && source.status === "accepted") || data.leads.some(lead => lead.source === "qbcc"))) return null;
  return <span className={cx("source-attribution")}><br />Source: <a href="https://www.data.qld.gov.au/dataset/qbcc-licensed-contractors-register/resource/25608781-b28c-44f8-8545-0ab18d84082f" target="_blank" rel="noreferrer" title="Open the QBCC dataset in a new tab">QBCC Licensed Contractors Register</a>, © State of Queensland (Queensland Building and Construction Commission), licensed under <a href="https://creativecommons.org/licenses/by/4.0/" target="_blank" rel="noreferrer" title="Open the licence in a new tab">CC BY 4.0</a>. Reformatted and filtered by Maintain Media; our review decisions are separate from the source. No QBCC endorsement.</span>;
}
function AbrAttribution({ engine }: { engine: DashboardController }) {
  if (!engine.data || engine.data.mode === "fixture" || !engine.data.sources.some(source => source.source === "abr" && source.status === "accepted")) return null;
  return <span className={cx("source-attribution")}><br />Source: <a href="https://abr.business.gov.au/Tools/BulkExtract" target="_blank" rel="noreferrer">Australian Business Register / ABN Lookup bulk extract</a>, under <a href="https://creativecommons.org/licenses/by/3.0/au/" target="_blank" rel="noreferrer">CC BY 3.0 Australia</a>. Normalised and compared by Maintain Media; no publisher endorsement. Registration changes do not prove when a business began or permission to contact it.</span>;
}
...
function LeadsView({ engine, hidden }: { engine: DashboardController; hidden: boolean }) {
  ...
  return <section id="view-leads" className={cx("view")} hidden={hidden} aria-labelledby="leads-title">
    ...
    <div className={cx("source-footnote")}><Icon name="info" /><p>ABR and QBCC records help find candidates. They do not establish revenue, buying intent or permission to contact. Contact values are masked in the worklist; authorised reviewers can inspect private captured evidence. <a href="/business-research-notice" target="_blank" rel="noreferrer">Business research notice</a>.<QbccAttribution engine={engine} /></p></div>
  </section>;
}
function RunsView({ engine, hidden }: { engine: DashboardController; hidden: boolean }) {
  const job: Job | null | undefined = engine.data?.active_job || (engine.running ? { state: "queued", source: engine.data?.settings.default_source || "all" } : engine.data?.latest_job);
  const active = jobActive(job);
  const guidance = job ? sourceJobGuidance(job) : null;
  return <section id="view-runs" className={cx("view")} hidden={hidden} aria-labelledby="runs-title">
    ...
    <div id="active-run" className={cx("notice", "run-notice")} hidden={!job} aria-live="polite">{job && guidance && <><Icon name={active ? "clock" : job.state === "complete" ? "check" : "info"} /><div><strong>{guidance.title}</strong><p>{sourceName(job.source)} · {humanize(job.phase || job.state)}{job.error_code ? ` · ${errorMessage(job.error_code, `${humanize(job.error_code)}. Review the reported check before retrying.`)}` : active ? ". This page updates automatically." : ". Refresh to see the latest results."}</p>{guidance.detail && <p>{guidance.detail}</p>}{job.source === "abr" && job.state === "complete" && typeof job.result?.record_count === "number" && <p>Accepted source records: {job.result.record_count.toLocaleString("en-AU")} · Source published: {dateLabel(job.result.source_published_at)}{job.result.publisher_extract_time && <> · Publisher extract time (as supplied): {job.result.publisher_extract_time}</>}</p>}</div></>}</div>
    <div className={cx("section-heading")}><div><h2>Recent runs</h2><p>Source receipts describe the accepted publication. Worklist reports are separate.</p></div><RefreshButton engine={engine} small /></div>
    <div className={cx("runs-table-wrap")}><table className={cx("runs-table")}><thead><tr><th scope="col">Run</th><th scope="col">Source</th><th scope="col">Status</th><th scope="col">Selected</th><th scope="col">Reports</th></tr></thead><tbody id="run-list">{engine.data?.runs.map(run => <tr key={run.run_id}><td><span className={cx("run-date")}>{dateLabel(run.started_at, true)}</span><span className={cx("run-id")} title={run.run_id}>{run.run_id.slice(0, 8)}</span></td><td>{sourceName(run.source)}</td><td><span className={cx("state-badge", `state-${run.status}`)}>{run.status === "complete" && <Icon name="check" />}{humanize(run.status)}</span>{run.source === "abr" && <p className={cx("field-hint")}>{sourceJobGuidance({ ...run, state: run.status }).detail}{run.error_code ? ` ${errorMessage(run.error_code, humanize(run.error_code))}` : ""}</p>}</td><td>{run.selected ?? "—"}</td><td>{run.reports && safeReportURL(run.reports.html) ? <div className={cx("report-actions")}>{(["html", "csv", "markdown"] as ReportKind[]).map(kind => {
    ...
    })}</div> : <span className={cx("table-muted")}>{run.source === "abr" ? "Source receipt only" : ["running", "pending", "queued"].includes(run.status) ? "Processing source" : "No current report"}</span>}</td></tr>)}</tbody></table>
  ...
  return <section id="view-setup" className={cx("view")} hidden={hidden} aria-labelledby="setup-title">
    ...
    <div className={cx("settings-layout")}><div><section className={cx("settings-section")} aria-labelledby="defaults-title"><h2 id="defaults-title">Run defaults</h2><p>Shared across this admin workspace. Changes apply to the next dashboard run.</p>
      <form id="settings-form" aria-busy={engine.loading || engine.saving} onSubmit={event => { event.preventDefault(); void engine.saveSettings(); }}>
        <div className={cx("form-field")}><label htmlFor="default-source">Data source</label><select id="default-source" name="default_source" disabled={disabled} value={engine.source} onChange={event => engine.editSettings(event.target.value as LeadSource, engine.cap)}><option value="all" disabled={engine.data?.mode !== "fixture"}>ABR + QBCC{engine.data?.mode !== "fixture" ? " · choose one source" : ""}</option><option value="abr">Australian Business Register (ABR)</option><option value="qbcc">QBCC contractor register</option></select><p className={cx("field-hint")}>{engine.data ? (engine.data.mode === "fixture" ? "Choose which sample source to process when you run the demo." : "Choose one source and save. Its current checks below control whether it can run. ABR starts with a reference baseline: zero new-business events and zero leads. Matching remains separate from source collection.") : "Connect the engine before choosing a data source."}</p></div>
      ...
      <section className={cx("settings-section")} aria-labelledby="sources-title"><h2 id="sources-title">Data sources</h2><p>Publication and processing dates come from the engine’s stored records.</p><div id="source-list" className={cx("source-list")}>{engine.data?.sources.length ? engine.data.sources.map(source => <article key={source.source} className={cx("source-item")}><div className={cx("source-item-heading")}><h3>{source.source === "abr" ? "Australian Business Register" : "QBCC contractor register"}</h3><span className={cx("state-badge")}>{humanize(source.status)}</span></div><p>Last processed: {dateLabel(source.last_success_at)}<br />Source published: {dateLabel(source.source_published_at)}{source.publisher_extract_time && <><br />Publisher extract time (as supplied): {source.publisher_extract_time}</>}</p>{engine.data!.mode !== "fixture" && <><p>Accepted records: {typeof source.record_count === "number" ? source.record_count.toLocaleString("en-AU") : "Not recorded"}</p><p>{sourceRunAllowed(engine.data, source.source as LeadSource) ? "Current checks permit a source run." : "A source run is not currently confirmed. Check the reasons and workspace readiness."}</p>{!!source.reason_codes?.length && <p>{source.reason_codes.map(code => errorMessage(code, humanize(code))).join(" ")}</p>}{source.source === "abr" && <p>{source.baseline === true ? "Starting baseline accepted. Its first import created zero new-business events and zero leads. " : "The first accepted publication establishes a reference baseline. "}{source.classification === "disabled" ? "Matching disabled: the separate 100-record accuracy review is still required." : "Matching status is not verified in this response."}</p>}</>}{source.source === "qbcc" ? <QbccAttribution engine={engine} /> : source.source === "abr" ? <AbrAttribution engine={engine} /> : null}</article>) : <p className={cx("field-hint")}>{engine.data ? (engine.data.mode === "fixture" ? "No accepted source snapshots yet. Run the demo to load sample sources." : "No accepted source snapshots yet. Review the current approval checks before running source collection.") : "Source information will appear when the engine connects."}</p>}</div></section>
