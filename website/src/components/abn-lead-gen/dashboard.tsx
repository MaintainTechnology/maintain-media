...
function RunButton({ engine }: { engine: DashboardController }) {
  ...
  return <button type="button" className={cx("button", "primary", "run-button")} disabled={!engine.data || active || engine.saving || engine.signingOut} onClick={() => void engine.beginRun()}><Icon name={active ? "clock" : "play"} /><span>{active ? "Run in progress" : "Run demo"}</span></button>;
}
...
function LeadsView({ engine, hidden }: { engine: DashboardController; hidden: boolean }) {
  ...
  return <section id="view-leads" className={cx("view")} hidden={hidden} aria-labelledby="leads-title">
    ...
    <div className={cx("demo-note")}><Icon name="info" /><p>You’re in demo mode. These are sample businesses. <a href="#setup">See what’s needed for live data <span aria-hidden="true">→</span></a></p></div>
    ...
    <div className={cx("section-heading")}><div><h2>Latest leads <span className={cx("count-label")} id="lead-count">{engine.data ? `(${leads.length})` : ""}</span></h2><p>Most recently qualified first. Select a business to inspect its details.</p></div><button type="button" className={cx("button", "secondary", "small", "export-latest")} disabled={!latest} onClick={() => { if (latest?.reports?.csv) void engine.openReport(latest.reports.csv, "csv"); }}><Icon name="download" />Export latest CSV</button></div>
...
function RunsView({ engine, hidden }: { engine: DashboardController; hidden: boolean }) {
  ...
  return <section id="view-runs" className={cx("view")} hidden={hidden} aria-labelledby="runs-title">
    ...
    <div className={cx("runs-table-wrap")}><table className={cx("runs-table")}><thead><tr><th scope="col">Run</th><th scope="col">Source</th><th scope="col">Status</th><th scope="col">Selected</th><th scope="col">Reports</th></tr></thead><tbody id="run-list">{engine.data?.runs.map(run => <tr key={run.run_id}><td><span className={cx("run-date")}>{dateLabel(run.started_at, true)}</span><span className={cx("run-id")} title={run.run_id}>{run.run_id.slice(0, 8)}</span></td><td>{sourceName(run.source)}</td><td><span className={cx("state-badge", `state-${run.status}`)}>{run.status === "complete" && <Icon name="check" />}{humanize(run.status)}</span></td><td>{run.selected ?? "—"}</td><td>{run.reports && safeReportURL(run.reports.html) ? <div className={cx("report-actions")}>{(["html", "csv", "markdown"] as ReportKind[]).map(kind => {
      ...
      return url ? <a key={kind} className={cx("button", "small", "secondary", "report-link")} href={url} data-kind={kind} target={kind === "html" ? "_blank" : undefined} rel={kind === "html" ? "noopener" : undefined} download={kind !== "html" || undefined} onClick={event => { event.preventDefault(); void engine.openReport(url, kind); }}><Icon name={kind === "html" ? "file" : "download"} />{kind === "html" ? "Report" : kind === "csv" ? "CSV" : "Markdown"}</a> : null;
      ...
      <div id="runs-empty" className={cx("empty-state")} hidden={!engine.data || !!engine.data.runs.length}><h3>Your first run starts here.</h3><p>Run the demo to process sample data and generate a review worklist.</p><RunButton engine={engine} /></div>
    </div>
  </section>;
}
function SetupView({ engine, hidden }: { engine: DashboardController; hidden: boolean }) {
  const budget = engine.data?.budget;
  const disabled = !engine.data || engine.saving || engine.signingOut;
  return <section id="view-setup" className={cx("view")} hidden={hidden} aria-labelledby="setup-title">
    <div className={cx("page-heading")}><div><h1 id="setup-title">Make the engine work for you.</h1><p>Set your run defaults and see how your workspace is connected.</p></div><span className={cx("mode-badge")}>Demo workspace</span></div>
    <div className={cx("settings-layout")}><div><section className={cx("settings-section")} aria-labelledby="defaults-title"><h2 id="defaults-title">Run defaults</h2><p>Shared across this admin workspace. Changes apply to the next dashboard run.</p>
      ...
      return <article key={item.id || `${item.label}-${index}`} className={cx("setup-item", ...(ready ? ["ready"] : []))}><Icon name={ready ? "check" : "info"} /><div><h3>{item.label}</h3><p>{item.detail}</p></div></article>;
    })}{!engine.data && <p className={cx("field-hint")}>Connecting to check engine readiness…</p>}</div></section><div className={cx("setup-explainer")}><h3>Ready for real businesses?</h3><p>The demo is ready to explore. Live collection needs approved source access, matching rules, provider setup and a completed release review.</p><p>Live connections are configured during deployment. This dashboard keeps outreach disabled.</p></div></aside></div>
  </section>;
}
...
export function LeadGenDashboard({ admin }: { admin: DashboardAdmin }) {
  ...
  if (engine.expired) return <div className={cx("dashboard", "session-ended")}><Image src="/brand/logo-darkbg.svg" alt="Maintain Media" width={168} height={34} style={{ height: "auto" }} /><h1>Your admin workspace is locked.</h1><p role="alert">{engine.error}</p><a className={cx("button", "primary")} href="/abn-lead-gen/sign-in">Sign in again <Icon name="arrow" /></a><Link href="/">Back to Maintain Media</Link></div>;
  ...
  return <div className={cx("dashboard", "app-shell")}>
    <aside className={cx("sidebar")}><a className={cx("brand")} href="#leads" aria-label="Maintain Media — latest leads"><Image src="/brand/logo-darkbg.svg" alt="Maintain Media" width={168} height={34} style={{ height: "auto" }} priority /></a><div className={cx("workspace-label")}>ABN Lead Engine</div>
      ...
      <div className={cx("sidebar-bottom")}><div className={cx("local-state")}><span id="connection-dot" className={cx("status-dot", engine.connected ? "ready" : engine.loading ? "connecting" : "failed")} /><span id="connection-label">{engine.connected ? "Engine connected" : engine.loading ? "Connecting" : "Engine unavailable"}</span></div><p>Private admin workspace<br />Sample businesses only</p><a href="#setup" className={cx("text-link")}>View setup <Icon name="arrow" /></a><Link href="/" prefetch={false} onNavigate={leaveWorkspace} className={cx("text-link", "website-link")}>Maintain Media website <Icon name="arrow" /></Link></div>
    </aside>
    <div className={cx("main-shell")}><header className={cx("topbar")}><div className={cx("breadcrumb")}>Workspace <span>/</span> <strong id="breadcrumb-current">{label}</strong></div><div className={cx("admin-controls")}><span className={cx("mode-badge")}>Demo data</span><span className={cx("admin-name")} title={admin.username}>{admin.displayName}</span><button id="admin-logout" type="button" className={cx("button", "secondary", "small")} disabled={engine.signingOut} onClick={() => void engine.logout()}><Icon name="logout" /><span>{engine.signingOut ? "Signing out…" : "Sign out"}</span></button></div></header>
      <div className={cx("content")}><div id="error-banner" className={cx("notice", "error-notice")} role="alert" hidden={!engine.error}><Icon name="info" /><p id="error-text">{engine.error}</p><button type="button" className={cx("button", "small", "secondary")} id="retry-button" disabled={engine.loading} onClick={() => void engine.refresh(true)}>Try again</button></div>
