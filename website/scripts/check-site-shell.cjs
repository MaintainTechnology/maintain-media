...
(async () => {
  ...
  try {
    ...
    assert.equal(await page.getByRole('region', { name: 'Account access', exact: true }).count(), 1);
    assert.equal(await page.evaluate(() => document.documentElement.scrollWidth > innerWidth), false);
    assert.deepEqual(errors, []);
    const result = { status: 'passed', checked_at: new Date().toISOString(), scope: 'Signed-out layout only. Does not verify Clerk account creation or authenticated admin workflows.', checks: ['Public mobile menu closes after navigation', 'Services, About and Contact retain header/footer and fit390px', 'Canonical Clerk sign-in has dedicated responsive chrome'], browser_errors: errors };
