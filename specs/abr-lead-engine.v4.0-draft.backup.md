...
**Version:** 4.0 · **Written:** 7 September 2026
...
CREATE TABLE contact_record (
  ...
  first_provenance_id UUID NOT NULL          -- makes an endpoint-without-provenance unrepresentable
    REFERENCES collection_provenance(provenance_id) DEFERRABLE INITIALLY DEFERRED,
...
**Phase 1 is conditional on Phase 0, and the measured volumes make that gate much sharper than
it first appeared.** The numbers, now measured rather than estimated:

- **QBCC Phase 0 alone: 11,080 leads = 185 weeks of worklist** at the 60/week cap, for 1–2
  developer-days, with street addresses and a regulator-verified revenue band.
- **The entire ABR pipeline adds ~130 leads/week** in the target geography, for 15–23
  developer-days, and 88% of its tier A is sole traders with no business name.

So the ABR pipeline costs roughly **ten times the build effort to supply a fraction of the same
VA's capacity, at lower quality.** State the implication plainly: **if the constraint is VA hours
rather than lead supply — and at 60 leads/week it is — Phase 1 does not need to be built at all
this year.** Build it when Phase 0's backlog is genuinely drawing down, or when the weekly
freshness of a GST-flip signal is proven to convert better than the QBCC list. Not before.

If four weeks of QBCC worklists produce no booked meeting, the problem is the offer or the script,
not the data — and a 20-million-record weekly pipeline will not fix it.
...
5. Extract the snapshot columns listed in §Data model. The parser must tolerate self-closing
   elements (`<GST ... />`) and must not depend on attribute order.
...
24a. **APP 7 — direct marketing.** This pipeline uses personal information (every sole trader,
    ...
    `do_not_market` keyed on ABN and are checked by the export gate alongside `spam_act_basis`,
...
25. **Fail closed, and the gate lives in the export step, not in the VA's judgement.**
    - No email endpoint is exported as sendable unless `record_level_pass = true` (limbs a–d)
      and no `do_not_market` row exists for the `lead_id`. **Limb e is asserted at send time by the
    ...
    - No phone endpoint is exported as callable unless a `dnc_wash` row exists for its
      ...
      through an ACMA-recognised Do Not Call Register access seeker or a licensed washing
      service; the account must be established **before** the first call, the per-tranche cost
...
- **Host:** a small VPS (4 vCPU / 8 GB / 80 GB NVMe, ~A$15–25/month) in an **Australian region**,
  plus object storage in an Australian region for snapshots and page captures. **The 80 GB local
  disk holds only the working set** — the two ZIPs (~1 GB), the current and prior snapshots, and
  Postgres — with a pre-run free-space check that aborts below 25 GB. The 24-month snapshot
  archive (R31a) lives in object storage, ESTIMATED at 300–500 MB per weekly snapshot ⇒ ~40–60 GB
  over 24 months ⇒ ~A$1–3/month. Measure the first snapshot and replace the estimate.
- **Estimated running cost:** infrastructure ~A$20–35/month; enrichment ~US$6–10 per 1,000
  records [ESTIMATE: SERP ~US$2–3 per 1,000 at 2–3 queries each, verification ~US$3.70 per 1,000;
  both vendor list prices, to be confirmed at signup]; DNC washing per the provider's tranche
  pricing, to be quoted before the first call.
  With the QLD/northern-NSW filter and the **MEASURED ~130 enriched records per week**, total is
  **A$24–40/month** excluding DNC washing. The R20 spend cap of A$150/month is a safety ceiling
  with ~4× headroom, not a budget. Infrastructure, not enrichment, is the dominant cost — which
  means **cost is not the reason to hesitate here; lead supply is.**
...
- [ ] `robots.txt` is honoured — a fixture site disallowing `/contact` is not fetched.
...
- [x] **DONE 7 Sep 2026.** The pre-build count was run against the 2026-09-03 extract
      (`Public01` + `Public11`, 2,050,400 records, ×10). Result: **tier A = 410/week national,
      60/week QLD; tier B = 210/week national, 70/week QLD + northern NSW.** GST dates confirmed
      usable (median backdating 2 days). The prior estimate of 5,000–6,000/week for tier A was
      **13× too high.** See the Appendix for the full funnel and composition. The script is at
      `deliverables/abr/count_tiers.py`.
- [ ] Re-run the same count on two further extracts to confirm the weekly figure is stable and
      not an artefact of one quiet week (the 7-day window is small — 41 sampled tier A records —
      so the ±1 sampling error on the national figure is roughly ±60/week).
...
2. **DNC washing provider and price.** R25 requires a wash receipt before any call. The access
   route (direct ACMA access-seeker registration versus a licensed washing service), the lead
   time to open the account, and the per-tranche price must be settled before the first call and
   added to the cost model. This is the only item that can block the phone channel entirely.
...
| Volume | **17,620/wk MEASURED** | **410/wk national, 60/wk QLD — MEASURED** | **11,080 total, VERIFIED** |
