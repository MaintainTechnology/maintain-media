"""Fixture downstream stop projection with durable claims and verified update receipts."""
...
def _authority(settings, service, claim):
    with transaction(settings) as conn:
        ...
        if any(mapping["location_id"] != "fixture" for mapping in mappings):
            raise DomainError("PROPAGATION_LOCATION_NOT_FIXTURE")
...
def _finish(settings, service, claim, receipt=None, error=None):
    with transaction(settings) as conn:
        service.authority(conn)
        row = conn.execute("SELECT * FROM propagation_outbox WHERE outbox_id=%s FOR UPDATE", (claim["outbox_id"],)).fetchone()
        now = service.now(conn)
        ...
        conn.execute("UPDATE propagation_outbox SET state=%s,completed_at=%s,receipt=%s,last_error_code=%s,next_attempt_at=%s,lease_owner=NULL,lease_until=NULL WHERE outbox_id=%s",
                     (state, now if receipt is not None else None, Jsonb(receipt) if receipt is not None else None,
                      error, None if receipt is not None else now+timedelta(seconds=min(300, 2**row["attempts"])), row["outbox_id"]))
...
def drain_propagation(settings, service, *, limit=60, provider=None):
    if settings.mode != "fixture" or service.settings.mode != "fixture":
        raise DomainError("LIVE_PROPAGATION_DISABLED", 403)
    if not 1 <= limit <= 60:
    ...
    for claim in _claim(settings, service, limit):
        try:
            ...
            for group in family:
                matches = provider.find_group(str(group))
                if len(matches) > 1:
            ...
            for remote_id, group_id in identities.items():
                ...
                if not remote or remote.get("group_id") != group_id or remote.get("payload", {}).get("group_id") != group_id:
                    raise DomainError("PROPAGATION_IDENTITY_CONFLICT")
                desired = blocked_projection(remote)
                _authority(settings, service, claim)
                if digest(remote["payload"]) != digest(desired):
                    provider.update(remote_id, desired, str(claim["outbox_id"]))
                after = provider.fetch(remote_id)
                if not after or after.get("group_id") != group_id or digest(after.get("payload", {})) != digest(desired):
            ...
            results.append(_finish(settings, service, claim, receipt={"mode": "fixture", "verified_remote_records": len(verified),
            ...
            results.append(_finish(settings, service, claim, error=code))
    return {"mode": "fixture", "notifications_sent": 0, "results": results}
