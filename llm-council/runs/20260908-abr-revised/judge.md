...
SPECIFICATION SCORE: 8.5/10. Weighted rubric for the supplied specification: correctness/evidence 8.5/10 x25% = 2.13; requirements/acceptance 8.8/10 x20% = 1.76; architecture/data integrity 8.8/10 x20% = 1.76; privacy/security/operability 8.4/10 x20% = 1.68; scope/delivery/traceability 7.9/10 x15% = 1.19; weighted total = 8.52/10, rounded to 8.5/10. This is a specification-readiness score only, not production certification, benchmark proof, procurement approval, or legal approval.

Plan 2 is stronger. It is more precise about the actual documentation defects: unchecked security checklist items, risky `OPEN/PENDING` gate wording, missing path/link validation, and the distinction between absent legacy rules as a production-classifier blocker versus a build-planning blocker. It also better consolidates acceptance traceability and release-gate semantics.

Plan 1 covers most required areas and has useful emphasis on blocker tables and checklist drift, but it is more verbose and less exact on path/link correction and gate wording. Its 8.4 specification score is defensible, though it slightly underuses the strong supplied contracts and data model. Plan 2's 8.6 is close, but slightly generous while CHK006/007/008/010/018/019 remain visibly unchecked in the supplied security checklist.

## Missing Steps
- Explicitly state the judge's final specification score separately from Plan 1 and Plan 2 scores.
- Add exact blocker format for each major defect: section/requirement, failure example, smallest correction.
- Normalize gate language from `OPEN/PENDING` to `pending/unapproved; live capability disabled`.
- Validate or relabel local references, especially `deliverables/abr/extract_new_abns.py`, `001-abr-lead-engine/spec.md`, `../abr-review-source-checks.md`, and skill-path references.
- Add a compact FR/R-to-task-to-acceptance matrix so R43/FR-043 is auditable without reading every artifact.
- Resolve security checklist drift by either checking CHK006/007/008/010/018/019 with evidence or leaving them unchecked with exact missing contracts.
- Add explicit safe-default examples for consent/channel separation, including deliverable email without reviewed basis, phone wash without call permission, and old pass overridden by newer fail/unknown/withdrawn.
- Keep historical verified assertions unverified unless reproduced in attached evidence.

## Contradictions
- The canonical spec and contracts appear to cover provenance, consent, action-time checks, suppression restore, pilot measurement, and traceability, but `security-and-readiness.md` leaves CHK006, CHK007, CHK008, CHK010, CHK018, and CHK019 unchecked.
- `OPEN/PENDING` release-gate wording can be misread as live capability being open, conflicting with the stated fail-closed defaults and production switches off.
- The canonical specification references the Spec Kit package as if present, while the brief says the claimed `deliverables/abr/extract_new_abns.py` path does not exist in the checkout.
- The source materials contain strong future implementation tasks, but none are implemented; the plan must not treat task coverage as executed evidence.
- The spec is documentation-ready for scoped build planning, but production classification remains blocked by the absent 30-rule corpus and missing provenance.

## Improvements
- Use Plan 2 as the base because it is clearer, more actionable, and better scoped.
- Retain Plan 1's blocker-table discipline and explicit challenge list.
- Lower the final independent specification score to 8.5/10 to reflect strong architecture and safety design while still penalizing visible checklist drift and traceability gaps.
- Consolidate phases to avoid excessive documentation churn: score, fix contradictions, tighten contracts, add traceability, publish owner-readable summary.
- Add exact examples for same-day ABR corrections, mixed ZIP generations, suppression restore, budget rollover, DNCR latest-result precedence, and action-time races.
- Keep all live/legal/vendor/capacity items as gated dependencies, not reasons to demand implementation evidence for a specification-quality review.
